...
jQuery(document).ready(function($) {
	...
	$("#searchform").submit(function() {
		...
		if($(this).attr('class') != 'search-form-error'){
			...
			if( mainWidth.width() >= 715 ){
				...
				if( getInputVal.val() == "" ) {
					mainBlock.animate({ width: 40 }, 1000 , "swing" );
	...
	if($('body.cuckoo-not-responsive').length == 0){
		...
		mob_check = $("#nav_wrap").find('.iphone-elements').css('display'),
		is_mobile_ele = mob_check != 'none' ? 0 : 28 ;
		topCount = !adminPanelTop ? 0 : is_mobile_ele;
